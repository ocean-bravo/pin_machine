...
OpenCv::BlobsOnImage detectBlobs(QImage img, QVariantMap options)
{
    ...
    try {
        ...
        db().insert("live_preview_image_adapt_threshold", mat_to_qimage_ref(adTr, QImage::Format_Grayscale8).copy());
...
double OpenCv::corr(QImage cap1, QImage cap2)
{
    ...
    //db().insert("live_preview_image_corr", mat_to_qimage_ref(res, QImage::Format_Grayscale8).copy());
